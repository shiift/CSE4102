...
eval(app(clos(Param, Body, Env), Arg), Env, Return, ReturnEnv) :- !,
	eval(Arg, Env, ArgVal, ArgEnv),
	env_bind(ArgEnv, Param, ArgVal, BindEnv),
	eval(Body, BindEnv, Return, ReturnEnv).
...
eval(let(Bindings, Body), Env, Return, Env) :- !,
	let_helper(Bindings, Env, LetEnv),
...
% special cases for built-in control predicates
...
to add */
let_helper([], Env, Env) :- !.
let_helper([[Name|Expr]|Tail], Env, ReturnEnv) :- !,
	eval(Expr, Env, ExprVal, EvalEnv),
	env_bind(EvalEnv, Name, ExprVal, BindEnv),
	let_helper(Tail, BindEnv, ReturnEnv).